// Validation for tools/index/wfc-adjacency.mjs — see
// docs/WFC-TILE-ADJACENCY-SPEC.md "Validation". Synthetic in-memory tile
// fixtures, same pattern as tools/index/test-geometry.mjs.
//
// Ported unmodified from vendor/geometric-connectivity-spec-eb8b72/tools/test-wfc-adjacency.mjs
// -- only the import path changed. These are the phase's own stated oracle
// (KICKOFF.md Phase 1), so the safest port keeps them byte-for-byte
// identical. None of these cases call computeGroupWFCData, so its
// async-ification for the sharp decode path (see wfc-adjacency.mjs) doesn't
