...
// an automated, exact-invariant oracle for tools/geometry-extractor.mjs,
...
// PNG decoding (including the bit-unpacking and PLTE/tRNS fixes) is
// exercised separately, against real extracted catalog files, in the
// decoder smoke-check run before this script was written — see the commit
// history. Splitting the two keeps this file about the geometry algorithm
// only and lets fixtures be built with exact pixel control.
//
...
// part of the contract. This is a deliberate, documented adjustment from
// "exact expected vertex list" in the original spec wording — still a real
// independently-reasoned oracle (each expected value is computed from the
// fixture's known geometry, not copied from a first run of the code under
// test), just tolerant of rotation/starting-vertex.

import assert from "node:assert/strict"
import { computeGeometry } from "./geometry-extractor.mjs"
